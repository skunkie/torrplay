...
func TestContentDirectory_Categories(t *testing.T) {
	...
	t.Run("search sorts before pagination", func(t *testing.T) {
		criteria, err := search.Parse(`(dc:title contains "Movie")`)
		require.NoError(t, err)

		didl, totalMatches, err := cd.Search(ctx, categoriesContainerID, criteria, 1, 1, xmltypes.CommaSeparatedStrings{"-dc:title"})
		require.NoError(t, err)
		assert.Equal(t, uint(4), totalMatches)
		require.Len(t, didl.Containers, 1)
		assert.Equal(t, "Movie A", didl.Containers[0].Title)

		_, _, err = cd.Search(ctx, categoriesContainerID, criteria, 0, 10, xmltypes.CommaSeparatedStrings{"+upnp:class"})
		assert.ErrorIs(t, err, contentdirectory.ErrInvalidSortCriteria)
	})
...
func TestPageBoundsDoesNotOverflow(t *testing.T) {
	start, end, ok := pageBounds(5, 1, ^uint(0))
	require.True(t, ok)
	assert.Equal(t, 1, start)
	assert.Equal(t, 5, end)

	_, _, ok = pageBounds(5, ^uint(0), 1)
	assert.False(t, ok)
}
