...
func TestBBoltDBService(t *testing.T) {
	t.Run("download and save data", func(t *testing.T) {
		ctx := t.Context()
	...
	t.Run("rejects invalid images", func(t *testing.T) {
		ctx := t.Context()
		dbPath := filepath.Join(t.TempDir(), "posters.db")
		s, err := NewBBoltDBService(dbPath)
		require.NoError(t, err)
	...
	t.Run("accepts BMP and AVIF", func(t *testing.T) {
		ctx := t.Context()
		dbPath := filepath.Join(t.TempDir(), "posters.db")
		s, err := NewBBoltDBService(dbPath)
		require.NoError(t, err)
	...
	t.Run("deduplicates saved data", func(t *testing.T) {
		dbPath := filepath.Join(t.TempDir(), "posters.db")
		s, err := NewBBoltDBService(dbPath)
		require.NoError(t, err)
		defer s.Close()
	...
	t.Run("sends cookies from jar", func(t *testing.T) {
		ctx := t.Context()
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			if r.URL.Path == "/login" {
...
// TestNewBBoltDBService verifies that NewBBoltDBService rejects an unusable path.
func TestNewBBoltDBService(t *testing.T) {
