...
export type Architecture = 'x64' | 'arm64' | 'armv7' | 'x86' | 'unknown';
...
export function getArchitecture(): Architecture {
  ...
  if (['arm', 'armv7', 'armv7l', 'armeabi-v7a'].includes(configuredArchitecture)) return 'armv7';
  if (['x64', 'x86_64', 'amd64'].includes(configuredArchitecture)) return 'x64';
  if (['x86', 'i386', 'i686', '386'].includes(configuredArchitecture)) return 'x86';

  if (typeof window === 'undefined' || !window.navigator) return 'unknown';

  const userAgent = (window.navigator.userAgent || '').toLowerCase();
  if (/arm64|aarch64/.test(userAgent)) return 'arm64';
  if (/armeabi-v7a|armv7l?/.test(userAgent)) return 'armv7';
  if (/x86_64|x64|amd64|win64/.test(userAgent)) return 'x64';
  if (/\bx86\b|\bi[3-6]86\b/.test(userAgent)) return 'x86';
  return 'unknown';
}
