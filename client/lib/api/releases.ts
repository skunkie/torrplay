...
export async function fetchLatestRelease(
  ...
  }: { forceRefresh?: boolean, timeoutMs?: number } = {}
...
export function classifyAsset(asset: GitHubReleaseAsset): ReleaseAssetInfo {
  ...
  if (/arm64|aarch64/.test(name)) {
    architecture = 'arm64';
  } else if (/armeabi-v7a|armv7l?/.test(name)) {
    architecture = 'armv7';
  } else if (/x86_64|x64|amd64/.test(name)) {
    architecture = 'x64';
  } else if (/\bx86\b|\bi[3-6]86\b/.test(name)) {
    architecture = 'x86';
  }
...
export function classifyReleaseAssets(
  ...
  const targetCompatible = compatible.filter(asset => isDesktop
    ? [
      'desktop-setup',
      'desktop-msi',
      'macos-dmg',
    ...
    : ['windows-service', 'linux-server-deb', 'linux-server-rpm'].includes(asset.type));
  ...
  switch (os) {
    case 'macos': {
      ...
      primaryAsset = targetCompatible.find(a => a.type === 'macos-dmg') || null;
      break;
    }
    case 'windows': {
      // Desktop clients update the local app; browser clients update the connected server.
      const exeAsset = targetCompatible.find(a => a.type === 'desktop-setup');
      const desktopMsiAsset = targetCompatible.find(a => a.type === 'desktop-msi');
      const serviceMsiAsset = targetCompatible.find(a => a.type === 'windows-service');

      if (isDesktop) {
        primaryAsset = exeAsset || desktopMsiAsset || null;
      } else {
        primaryAsset = serviceMsiAsset || null;
      }
      break;
    }
    case 'linux': {
      const appImage = targetCompatible.find(a => a.type === 'linux-appimage');
      const desktopDeb = targetCompatible.find(a => a.type === 'linux-desktop-deb');
      const desktopRpm = targetCompatible.find(a => a.type === 'linux-desktop-rpm');
      const serverDeb = targetCompatible.find(a => a.type === 'linux-server-deb');
      const serverRpm = targetCompatible.find(a => a.type === 'linux-server-rpm');
      primaryAsset = isDesktop
        ? appImage || desktopDeb || desktopRpm || null
        : serverDeb || serverRpm || null;
      break;
    }
    default: {
      primaryAsset = null;
  ...
  for (const asset of targetCompatible) {
