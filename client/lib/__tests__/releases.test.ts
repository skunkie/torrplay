...
describe('releases', () => {
  const mockAssets: GitHubReleaseAsset[] = [
    ...
    {
      name: 'torrplay_1.1.0_amd64.deb',
      browser_download_url: 'https://github.com/torrplay/torrplay/releases/download/v1.1.0/torrplay_1.1.0_amd64.deb',
      size: 18000000,
    },
    {
  ...
  describe('classifyReleaseAssets', () => {
    it('normalizes 32-bit Android architecture names', () => {
      expect(classifyAsset({
        name: 'torrplay-android-armeabi-v7a.apk',
        browser_download_url: 'https://example.com/torrplay-android-armeabi-v7a.apk',
      }).architecture).toBe('armv7');
      expect(classifyAsset({
        name: 'torrplay-android-x86.apk',
        browser_download_url: 'https://example.com/torrplay-android-x86.apk',
      }).architecture).toBe('x86');
    });
    ...
    it('selects Windows Desktop Setup .exe as primary asset on Windows desktop', () => {
      ...
      expect(result.secondaryAssets.some(a => a.type === 'windows-service')).toBe(false);
    });

    it('selects the Windows service package for a browser-connected server', () => {
      const result = classifyReleaseAssets(mockAssets, 'windows', false, 'x64');
      expect(result.primaryAsset).not.toBeNull();
      expect(result.primaryAsset?.type).toBe('windows-service');
      expect(result.secondaryAssets.some(a => a.type === 'desktop-setup')).toBe(false);
    });
    ...
    it('selects a server package for a Linux browser connection', () => {
      const result = classifyReleaseAssets(mockAssets, 'linux', false, 'x64');
      expect(result.primaryAsset).not.toBeNull();
      expect(result.primaryAsset?.type).toBe('linux-server-deb');
      expect(result.secondaryAssets.some(a => a.type === 'linux-appimage')).toBe(false);
    });

    it('does not offer a desktop DMG for a macOS browser connection', () => {
      const result = classifyReleaseAssets(mockAssets, 'macos', false, 'arm64');
      expect(result.primaryAsset).toBeNull();
      expect(result.secondaryAssets).toEqual([]);
    });
  ...
  describe('fetchLatestRelease', () => {
    ...
    it('aborts a release request after the configured timeout', async () => {
      vi.useFakeTimers();
      ...
      const request = expect(fetchLatestRelease(
        'https://example.com/releases/latest',
        { forceRefresh: true, timeoutMs: 50 }
