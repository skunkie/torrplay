...
describe('platform', () => {
  ...
  it('detects architecture from user agent', () => {
    ...
    Object.defineProperty(window.navigator, 'userAgent', {
      value: 'Mozilla/5.0 (Linux; Android 12; armeabi-v7a) AppleWebKit/537.36',
      configurable: true,
    });
    expect(getArchitecture()).toBe('armv7');

    Object.defineProperty(window.navigator, 'userAgent', {
      value: 'Mozilla/5.0 (Linux; Android 9; x86) AppleWebKit/537.36',
      configurable: true,
    });
    expect(getArchitecture()).toBe('x86');
  });
  ...
  it('uses the build architecture when configured', () => {
    ...
    vi.stubEnv('NEXT_PUBLIC_APP_ARCH', 'armeabi-v7a');
    expect(getArchitecture()).toBe('armv7');

    vi.stubEnv('NEXT_PUBLIC_APP_ARCH', '386');
    expect(getArchitecture()).toBe('x86');
  });
